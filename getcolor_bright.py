...
def extract_quantized_colors(image_path, num_colors=8):
    """
    Extracts quantized colors from the image, prioritizing bright or saturated colors for the primary color.
    ...
    with Image.open(image_path) as img:
        ...
        if len(valid_colors) < 3:
            valid_colors += [(255, 255, 255)] * (3 - len(valid_colors))

        # Sort by brightness and saturation, prioritizing the brightest/saturated color as the primary color
        valid_colors.sort(key=lambda c: (color_brightness(c), color_saturation(c)), reverse=True)
        primary_color = valid_colors[0]
        secondary_colors = valid_colors[1:3]
