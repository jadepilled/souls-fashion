...
def create_image_with_colors(image_path, primary_color, secondary_colors, output_path):
    """Creates a 500x500 image with a centered and resized image, scaling it up if too small, and a 120px bottom bar showing the primary and secondary colors."""
    with Image.open(image_path) as img:
        # Calculate scaling for images that are too small
        target_size = (500, 380)
        img_width, img_height = img.size
        scale_factor = max(target_size[0] / img_width, target_size[1] / img_height)

        # If the image is smaller than the target size, scale it up
        if img_width < target_size[0] or img_height < target_size[1]:
            new_width = int(img_width * scale_factor)
            new_height = int(img_height * scale_factor)
            img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)  # Replaced ANTIALIAS with LANCZOS

        # Ensure image fits within the 500x380 canvas
